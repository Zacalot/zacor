    ...
    fn invoke(
        ...
        match invoke_package_local(
            ...
            &mut |event| match event {
                InvocationEvent::Record(record) => on_output(record),
                InvocationEvent::Progress(_) | InvocationEvent::Message { .. } => Ok(()),
            },
        ) {
...
pub fn invoke_local_with_events(
    home: &Path,
    package: &str,
    command_path: &str,
    parsed_flags: &BTreeMap<String, String>,
...
"#,
    ...
    #[test]
    fn protocol_fixture_emits_events() {
        if std::env::var("ZR_DISPATCH_EVENT_FIXTURE").ok().as_deref() != Some("1") {
            return;
        }

        println!(r#"{{"type":"progress","fraction":0.25}}"#);
        println!(r#"{{"type":"output","record":{{"value":"hello"}}}}"#);
        println!(r#"{{"type":"done","exit_code":0,"error":"fixture done error"}}"#);
        std::io::stdout().flush().unwrap();
        std::process::exit(0);
    }

    #[test]
    fn invoke_local_with_events_streams_progress_output_and_done_error() {
        let home = test_util::temp_home("dispatch-events");
        let package_name = "fixture";
        let version = "1.0.0";
        let run = "cargo test -p zr-dispatch --lib -- --exact dispatch::tests::protocol_fixture_emits_events --nocapture";
        ...
        let exit_code = invoke_local_with_events(
            home.path(),
            package_name,
            "default",
            &BTreeMap::new(),
        ...
        unsafe {
            match original_flag {
                ...
                None => std::env::remove_var("ZR_DISPATCH_EVENT_FIXTURE"),
            }
        }
        ...
        assert_eq!(
            events,
            vec![
                InvocationEvent::Progress(0.25),
                InvocationEvent::Record(serde_json::json!({"value": "hello"})),
                InvocationEvent::Message {
                    level: InvocationMessageLevel::Error,
