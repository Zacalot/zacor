...
use zacor_host::session::{
    InputSource, OutputHandler, OutputMessageLevel, SessionConfig, StdioTransport, run_session,
};
...
impl OutputHandler for RenderingOutputHandler<'_> {
    ...
    fn on_message(&mut self, level: OutputMessageLevel, text: &str) {
        match level {
            OutputMessageLevel::Info => eprintln!("{text}"),
            OutputMessageLevel::Warning => eprintln!("warning: {text}"),
            OutputMessageLevel::Error => eprintln!("error: {text}"),
        }
    }
...
pub(crate) struct CallbackOutputHandler<'a> {
    on_event: &'a mut dyn FnMut(InvocationEvent) -> std::result::Result<(), String>,
    error: Option<String>,
}
...
impl OutputHandler for CallbackOutputHandler<'_> {
    fn on_output(&mut self, record: &Value) {
        if self.error.is_none()
            && let Err(error) = (self.on_event)(InvocationEvent::Record(record.clone()))
        {
    ...
    fn on_progress(&mut self, fraction: f64) {
        if self.error.is_none()
            && let Err(error) = (self.on_event)(InvocationEvent::Progress(fraction))
        {
            self.error = Some(error);
        }
    }
    ...
    fn on_message(&mut self, level: OutputMessageLevel, text: &str) {
        if self.error.is_none()
            && let Err(error) = (self.on_event)(InvocationEvent::Message {
                level: match level {
                    OutputMessageLevel::Info => InvocationMessageLevel::Info,
                    OutputMessageLevel::Warning => InvocationMessageLevel::Warning,
                    OutputMessageLevel::Error => InvocationMessageLevel::Error,
                },
                text: text.to_string(),
            })
        {
