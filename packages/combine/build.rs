fn main() {
    ...
    PackageSpec::from_cargo("combine")
        .command(
            CommandSpec::named("append")
                .description("Append records from second source")
                .args(&[file_arg.clone(), records_arg.clone()])
                .input(InputKind::Jsonl)
                .output(OutputSpec::table(&[])),
        )
        .command(
            CommandSpec::named("prepend")
                .description("Prepend records from second source")
                .args(&[file_arg.clone(), records_arg.clone()])
                .input(InputKind::Jsonl)
                .output(OutputSpec::table(&[])),
        )
        .command(
            CommandSpec::named("merge")
                .description("Merge fields row-by-row")
                .args(&[file_arg.clone()])
                .input(InputKind::Jsonl)
                .output(OutputSpec::table(&[])),
        )
        .command(
            CommandSpec::named("join")
                .description("SQL-style join on key field")
                .args(&[
                    file_arg.clone(),
        ...
        .command(
            CommandSpec::named("zip")
                .description("Pair records element-by-element")
                .args(&[file_arg.clone()])
                .input(InputKind::Jsonl)
                .output(OutputSpec::table(&[])),
        )
